...
server <- function(input, output) {
    ...
    sliderValues <- reactive({
        data.frame(
           Attribute= c("Team","Period","Difference","Probability"),
            ...
            round(df_prob%>%filter(period==as.numeric(input$period1), difference==as.numeric(input$Diff), team==input$team1)%>%select(probability), 3))),
    ...
    output$BarPlot <- renderPlot({
        game_id2=end_period%>%
            ...
            select(game.id)
        first_period12=end_period%>%
            filter(Period==as.numeric(input$period1), Diff.Points==as.numeric(input$Diff))%>%
            select(result)
        last_period12=semi_join(end_period, game_id2, by='game.id')%>%
            filter(Period==4)%>%
            select(result)
        ...
        ggplot(frame_, aes(x=Var2, y=Freq))+geom_col()+geom_text(aes(label=Freq), vjust=0)+xlab('Winning Team')+ylab('Number of Games')+ggtitle('Frequency of games for given parameters')
    })
