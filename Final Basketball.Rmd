...
Home.Points: Cumulative points scored by home team at the end of each period 
Away.Points: Cumulative points scored by away team at the end of each period 
Period.Diff: Home points minus Away points for given period (not cumulative)
...
```
The probability grows almost at the same rate in the interval from 1 to 10, and sharply slows dowMn after that.
...
#Value of additional Point difference 
```{r}
home=c()
away=c()
...
```
To get a better feeling about the probabilities obtained, I decided to regress probabilities on point difference for each combination of team and period. Worth to mention, all coefficients are highly significant. You can find the results on the plot below.
```{r}
...
```
On average, an additional point difference increases the probability of win by 2% for home team and by 2.5% for away team. Coeffients are higher for away team than for home team in all 3 periods, implying that when away team wins the period, an additional point in point difference will increase the chances for it to win with higher rate compared to the rate of change when home team wins the period. This may sound strange at first, however, when we think of this phenomenon from home advantage perspective, everything becomes clear. In the initial findigs, we saw that probability for home team when there is 1 point difference between teams is always higher than that of away team. At the same time, when the difference increses probability of away team approches to probability of home team. Thus, when we regress probabilities on point differences, slope for away team will be higher than that of home team.
One can also observe that the biggest change in probability for both teams occurs in 2nd period. This can mean that this period is the most important one in determing the winner of the whole game. Coaches may need to concentrate to score more in this period than in other ones. 
